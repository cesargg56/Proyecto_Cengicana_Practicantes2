...
$config = lab_generic_analysis_config('aguas-resistividad');
if (!$config) {
    lab_forbidden('El formulario de resistividad de aguas no esta configurado.');
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $campos = ['lote', 'numero_laboratorio', 'lectura_resistividad'];
    $resultados = [];
    ...
    if ($fecha === '' || $analista === '') {
        ...
        for ($fila = 0, $total = lab_post_row_count($campos); $fila < $total; $fila++) {
            ...
            $lecturaResistividad = lab_post_float('lectura_resistividad', $fila);
            ...
            try {
                ...
                if (empty($destino['id_tipo_analisis'])) {
                    throw new RuntimeException('No se pudo identificar el tipo de analisis para resistividad.');
                }
                ...
                $row = [
                    ...
                    'lectura_resistividad' => $lecturaResistividad,
                ];
                ...
                labFormularioGuardarVersion($idFormulario, 'inicial', $analista, 'Version enviada desde el formulario de analisis de resistividad.');
                ...
                $resultados[] = [
                    'exito' => true,
                    'mensaje' => 'Resistividad guardada correctamente.',
                ];
            } catch (Throwable $e) {
