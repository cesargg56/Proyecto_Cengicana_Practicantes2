<?php
	...
	if (!empty($_GET['id'])) {
		...
		$sql_nombre="SELECT nombre_participantes FROM participantes WHERE participantes.id=$id";
		...
		$participante=$fila_nombre['nombre_participantes'];
		$sql = "DELETE FROM participantes WHERE id=$id";
		$resultado = mysqli_query($mysqli, $sql) or $error=mysqli_error($mysqli);
		if(strstr($error, 'parent row: a foreign')<>FALSE) $error="El participantes está inscrito en otros cursos";
		mysqli_close($mysqli);
	}
	else
	{
		...
		<div class="container">
			<div class="row">
				<div class="row alert alert-info" style="text-align:center">
					<?php if($resultado) { ?>
						<h3>REGISTRO <strong><?php echo strtoupper($participante);?></strong> ELIMINADO</h3>
