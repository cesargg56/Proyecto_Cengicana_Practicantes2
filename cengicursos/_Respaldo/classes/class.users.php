...
/**
 * Clase para manejo de los usuarios
 *
...
class users{
	...
	public function consultar_visibles(){
		...
		$sql="SELECT id, nombre FROM cengi_cursos.users";
	...
	public function getCursoByNombre($nombre){
		$db = Database::getInstancia();
		$mysqli = $db->getConnection();
		$sql="SELECT id, nombre FROM cengi_cursos.users WHERE nombre LIKE '%$nombre%' ORDER BY nombre";
